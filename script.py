...
# Quick and dirty proof-of-concept script
# This script only works in object mode!!!
...
tile_infos = create_tile_infos(width, height, len(colors), 128, colors)
...
for color, faces in color_face_map.items():
    for face in faces:
        ...
        uvs = [Vector((v.x / width, v.y / height)) for v in uvs]
        i_uvs = iter(uvs)
        for vert_idx, loop_idx in zip(face.vertices, face.loop_indices):
